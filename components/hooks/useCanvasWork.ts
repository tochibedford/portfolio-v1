...
export const useCanvasWork = (canvasRef: RefObject<HTMLCanvasElement>, ballNumber: number, highlightRadius: number) => {
    useEffect(() => {
        //begin canvas logic
        const gravity = 0.0;
        const friction = 1;
        ...
        class Particle {
            ...
            update = (particles: Particle[], canvas: HTMLCanvasElement) => {
                ...
                if (getDistance(this.x, this.y, mouse.x, mouse.y) < highlightRadius) {
                    this.fill += 0.1
